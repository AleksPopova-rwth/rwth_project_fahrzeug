...
public:
	...
	{
		this->VAktion<T>::p_pListe.push_back(std::move(p_objekt));
	}
};
...
public:
	...
	void vAusfuehren() override
	{
		this->VAktion<T>::p_pListe.push_front(std::move(p_objekt));
	}
};
...
public:
	VErase() = default;
	VErase(list<T> &liste, iterator &obj) :
			VAktion<T>(liste), p_it(std::move(obj)) {
	}
