...
bool bStreetOk(const string &sName, bool bNeu) {
	...
	int iPos = sName.find_first_not_of(
			"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890_-");
	if (iPos > 0) {
		AfxMessageBox(
				CString(
						"Wegname darf nur Buchstaben, Ziffern, -_ und keine Blanks enthalten."));
		return false;
	}
	if (bNeu) {
		if (sName.size() > 10) {
...
SIMUCLIENT_API bool bInitialisiereGrafik(int sizeX, int sizeY,
	...
	if (iCount < 0) {
		AfxMessageBox(
				CString(
						"Connect nicht moeglich! (Firewall?) Grafikausgaben werden unterdrueckt."),
				false);
		return false;
	}
	stringstream msg;
...
bool x_bSetStreet(int length, int numPoints, int *points_xy) {
	...
	if (numPoints < 2) {
		AfxMessageBox(
				CString("Mindestens 2 Punkte fuer Strasse erforderlich."));
		return false;
	}
		...
		for (int i = 0; i < numPoints * 2; i += 2) {
			...
			if ((iX > 0) && (iX < iXSize))
				if ((iY > 0) && (iY < iYSize)) {
					...
					AfxMessageBox(
							CString(
									"Strassenpunkt ausserhalb des Verkehrsplans."));
					return false;
				}
			else {
				AfxMessageBox(
						CString(
								"Strassenpunkt ausserhalb des Verkehrsplans oder falsche Anzahl."));
				return false;
			}
		}
		msg << "#";
...
bool x_bSet(char cTyp, double rel_position, double speed, double tank = 0.0) {
	...
	int iPos = sFzName.find_first_not_of(
			"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz1234567890_-");
	if (iPos > 0) {
		AfxMessageBox(
				CString(
						"Fahrzeugname darf nur Buchstaben, Ziffern, -_ und keine Blanks enthalten."));
		return false;
	}
	...
	if (speed > 300.0) {
		AfxMessageBox(
				CString(
						"PKW sind keine Formel1-Fahrzeuge. Geschwindigkeit > 300 km/h"));
		return false;
	}
	if (tank < 0.0) {
	...
	if (tank > 999.9) {
		AfxMessageBox(
				CString(
						"Versteckte Tanks nicht erlaubt. Tankinhalt nur < 1000 l"));
		return false;
	}
		...
		if (bStreetOk(sSName, false)) {
			if (cTyp == 'c') {
				msg << "sc " << sFzName << " " << sSName
						<< setiosflags(ios::fixed) << " " << setw(7)
						<< setprecision(4) << rel_position << " " << setw(6)
						<< setprecision(1) << speed << " " << setw(6)
						<< setprecision(1) << tank << "#";
			} else if (cTyp == 'b') {
				msg << "sb " << sFzName << " " << sSName
						<< setiosflags(ios::fixed) << " " << setw(7)
						<< setprecision(4) << rel_position << " " << setw(6)
						<< setprecision(1) << speed << "#";
			} else
				return false;
			ClientSocket->Send(msg);
			vSleep(50);
...
SIMUCLIENT_API bool bZeichneStrasse(const char *way_to_name,
		const char *way_back_name, int length, int numPoints, int *points_xy) {
	sHName = string(way_to_name);
	sRName = string(way_back_name);
	return x_bSetStreet(length, numPoints, points_xy);
}

SIMUCLIENT_API bool bZeichneStrasse(const string &way_to_name,
		const char *way_back_name, int length, int numPoints, int *points_xy) {
	sHName = way_to_name;
	sRName = string(way_back_name);
	return x_bSetStreet(length, numPoints, points_xy);
...
SIMUCLIENT_API bool bZeichneStrasse(const char *way_to_name,
		const string &way_back_name, int length, int numPoints,
		int *points_xy) {
	sHName = string(way_to_name);
...
SIMUCLIENT_API bool bZeichnePKW(const string &carname, const char *streetname,
		double rel_position, double speed, double tank) {
	sFzName = carname;
	sSName = string(streetname);
	return x_bSet('c', rel_position, speed, tank);
}

SIMUCLIENT_API bool bZeichnePKW(const string &carname, const string &streetname,
		double rel_position, double speed, double tank) {
	sFzName = carname;
	sSName = streetname;
	return x_bSet('c', rel_position, speed, tank);
...
SIMUCLIENT_API bool bZeichneFahrrad(const string &bikename,
		const string &streetname, double rel_position, double speed) {
	sFzName = bikename;
	sSName = streetname;
...
SIMUCLIENT_API bool bZeichneFahrrad(const string &bikename,
		const char *streetname, double rel_position, double speed) {
	sFzName = bikename;
	sSName = string(streetname);
	return x_bSet('b', rel_position, speed);
}

SIMUCLIENT_API bool bZeichnePKW(const char *carname, const char *streetname,
		double rel_position, double speed, double tank) {
...
SIMUCLIENT_API bool bZeichnePKW(const char *carname, const string &streetname,
		double rel_position, double speed, double tank) {
	sFzName = string(carname);
	sSName = streetname;
	return x_bSet('c', rel_position, speed, tank);
...
SIMUCLIENT_API bool bZeichneFahrrad(const char *bikename,
		const char *streetname, double rel_position, double speed) {
